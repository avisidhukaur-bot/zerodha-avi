...
_ntfy_quota_exceeded_date = ""

def send_ntfy(message: str) -> bool:
    """Sends a push notification to mobile via ntfy.sh."""
    global _ntfy_quota_exceeded_date
    import datetime
    today_str = datetime.date.today().isoformat()
    if _ntfy_quota_exceeded_date == today_str:
        return False
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            _ntfy_quota_exceeded_date = today_str
            _log(f"Ntfy.sh daily message quota reached (HTTP 429). Muting Ntfy alerts for today.", "WARN")
            return False
        else:
...
def alert_block_toggled(block_number: int, side_type: str, is_enabled: bool) -> bool:
    """Sent when a block (CALL/PUT/BOTH) is Enabled or Paused."""
    status_str = "🟢 ENABLED (ACTIVE)" if is_enabled else "⏸️ PAUSED (ENGINE STOPPED)"
    msg = (
        f"<b>⚙️ BLOCK STATUS UPDATED</b>\n"
        f"Block #{block_number} | Side: <b>{side_type}</b>\n"
        f"New State: {status_str}\n"
        f"Time: {_ist_now()}"
    )
    return send(msg, alert_key=f"toggle_{block_number}")


def alert_strike_lots_updated(block_number: int, strike_price: int, option_type: str, old_lots: int, new_lots: int, sync_order: str = "") -> bool:
    """Sent when lot size for a strike is updated."""
    order_info = f"\nBroker Sync: {sync_order}" if sync_order else ""
    msg = (
        f"<b>🔢 STRIKE LOTS UPDATED</b>\n"
        f"Block #{block_number} | {strike_price} {option_type}\n"
        f"Lots: {old_lots} ➔ <b>{new_lots} Lots</b>{order_info}\n"
        f"Time: {_ist_now()}"
    )
...
# ─────────────────────────────────────────────────────────────────────────────
# ALERT 11: EQUITY 200 DMA EMERGENCY ALERT
# ─────────────────────────────────────────────────────────────────────────────

def alert_equity_200dma_breach(
    red_alerts: list,
    total_count: int,
    scan_time_label: str = "10:00 AM Scan"
) -> bool:
    """
    Sends emergency Telegram alert when any long-term equity stock falls below its 200 DMA.
    """
    n_alerts = len(red_alerts)
    safe_count = total_count - n_alerts
    
    lines = [
        f"🚨 <b>ZERODHA OS — EQUITY 200 DMA EMERGENCY ALERT</b>",
        f"⏰ <b>Scan Trigger:</b> {scan_time_label} ({_ist_now()})",
        "=" * 32,
        f"⚠️ <b>{n_alerts} Equity Stock(s) TRADING BELOW 200 DMA:</b>\n"
    ]
    
    for r in red_alerts:
        diff_sign = "+" if r['diff'] >= 0 else ""
        lines.append(
            f"🔴 <b>{r['symbol']}</b> ({r.get('name', r['symbol'])})\n"
            f"  • <b>LTP:</b> ₹{r['ltp']:,.2f}\n"
            f"  • <b>200 DMA:</b> ₹{r['dma_200']:,.2f}\n"
            f"  • <b>Variance:</b> {diff_sign}₹{r['diff']:,.2f} ({r['diff_pct']:.2f}%)\n"
            f"  • <b>Action:</b> Review / Trend Exit Candidate\n"
        )
        
    lines.append("=" * 32)
    lines.append(f"📊 <b>Holdings Status:</b> {safe_count} Safe 🟢 | {n_alerts} Alert 🔴 (Total: {total_count})")
    
    msg = "\n".join(lines)
    return send(
        msg,
        alert_key=f"equity_200dma_{datetime.now(IST).strftime('%Y-%m-%d_%H')}",
        cooldown_sec=300,
        force=True
    )
...
# ─────────────────────────────────────────────────────────────────────────────
# MASTER NIFTY REGIME ALERTS (Brick 5)
# ─────────────────────────────────────────────────────────────────────────────
...
def alert_regime_shift(
    ...
    anchor: float,
    buffer: float,
    closed_opposing_count: int,
    deployed_aligned_count: int,
    closed_details: list = None
) -> bool:
    """Sends alert when Master Nifty Anchor regime shifts and Kill & Flip executes."""
    closed_details = closed_details or []
    icon = "🟢" if new_regime == "BULLISH" else "🔴"
    regime_title = "🟢 BULLISH (PUT SELLING ACTIVE)" if new_regime == "BULLISH" else "🔴 BEARISH (CALL SELLING ACTIVE)"
    muted_side = "🔴 CALL SELLING (MUTED / CLOSED)" if new_regime == "BULLISH" else "🟢 PUT SELLING (MUTED / CLOSED)"
    
    lines = [
        f"{icon} <b>MASTER NIFTY REGIME SHIFT DETECTED</b> {icon}",
        f"━━━━━━━━━━━━━━━━━━━━━━━",
        f"<b>Shift:</b> {old_regime} ➔ <b>{new_regime}</b>",
        f"<b>Active Regime:</b> {regime_title}",
        f"<b>Muted / Dormant:</b> {muted_side}",
        f"<b>Live Nifty Spot:</b> ₹{spot:,.2f}",
        f"<b>Master Anchor:</b> ₹{anchor:,.2f} (Buffer: ±{buffer:.1f} pts)",
        f"━━━━━━━━━━━━━━━━━━━━━━━",
        f"⚡ <b>KILL & FLIP EXECUTION SUMMARY:</b>",
        f"• Opposing Legs Covered & Closed: <b>{closed_opposing_count}</b>",
        f"• Trend-Aligned Strikes Deployed: <b>{deployed_aligned_count}</b>",
    ]
    if closed_details:
        lines.append(f"\n<b>Closed Opposing Positions:</b>")
        for d in closed_details:
            lines.append(f"  ❌ {d}")
            
    lines.append(f"\n⏰ <i>Timestamp: {_ist_now()}</i>")
    msg = "\n".join(lines)
    return send(msg, alert_key=f"regime_shift_{new_regime}_{int(time.time() // 60)}", cooldown_sec=10, force=True)


def alert_regime_anchor_updated(anchor: float, buffer: float, spot: float, mode: str) -> bool:
    """Sends alert when Master Nifty Anchor is manually updated or locked via UI."""
    regime = "🟢 BULLISH (PUT SELLING)" if spot >= anchor else "🔴 BEARISH (CALL SELLING)"
    msg = (
        f"🎯 <b>MASTER NIFTY ANCHOR UPDATED</b> 🎯\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"<b>New Master Anchor:</b> ₹{anchor:,.2f}\n"
        f"<b>Hysteresis Buffer:</b> ±{buffer:.1f} pts\n"
        f"<b>Live Spot Price:</b> ₹{spot:,.2f}\n"
        f"<b>Governor Mode:</b> {mode}\n"
        f"<b>Calculated Regime:</b> {regime}\n"
        f"⏰ <i>Timestamp: {_ist_now()}</i>"
    )
    return send(msg, alert_key=f"anchor_updated_{int(anchor)}", cooldown_sec=5, force=True)
