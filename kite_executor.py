...
# Force all socket operations to use IPv4 to prevent Zerodha Kite API IP whitelist rejections (IPv6)
try:
...
class KiteExecutor:
    ...
    def _handle_kite_exception(self, e: Exception) -> None:
        """Helper to handle token exceptions and invalidate session if needed."""
        is_token_err = False
        if isinstance(e, exceptions.TokenException) or isinstance(e, exceptions.PermissionException):
            is_token_err = True
        else:
            msg = str(e).lower()
            if (
        ...
        if is_token_err:
            prev_status = db.get("kite_login_status", "")
            print("[EXECUTOR] Token/Permission exception detected. Invalidating token session.")
            self.access_token = None
            self.is_logged_in = False
            self.login_time = None
            db.set("kite_access_token", "")
            db.set("kite_is_logged_in", "False")
            db.set("kite_login_status", "EXPIRED")

            # Send Telegram alert once if status changed to EXPIRED
            if prev_status != "EXPIRED":
                try:
                    import telegram_bot as tg
                    tg.send(
                        "⚠️ <b>ZERODHA SESSION EXPIRED</b> ⚠️\n"
                        "Zerodha API returned insufficient permission / session expired.\n"
                        "<b>Action Required:</b> Please log in again from the Dashboard to resume live tracking/trading."
                    )
                except Exception as tg_err:
                    print(f"[EXECUTOR] Error sending token expiry alert: {tg_err}")
    ...
    def get_nifty_spot(self, force_refresh: bool = False) -> float:
        """
        Unified Nifty 50 Spot price fetcher across Zerodha Kite Connect API.
        Fetches 'NSE:NIFTY 50' with fallbacks (quote API -> cached -> yahoo ^NSEI -> DB).
        """
        now = time.time()
        if not force_refresh and hasattr(self, "_cached_spot") and (now - getattr(self, "_cached_spot_ts", 0)) < 3.0:
            if self._cached_spot > 0:
                return self._cached_spot

        spot = 0.0
        if self.ensure_logged_in() and self.kite:
            try:
                res = self.kite.ltp(["NSE:NIFTY 50"])
                if "NSE:NIFTY 50" in res:
                    spot = float(res["NSE:NIFTY 50"].get("last_price", 0.0))
            except Exception as e:
                print(f"[EXECUTOR] Error fetching NSE:NIFTY 50 via ltp(): {e}")
            
            if spot <= 0:
                try:
                    qres = self.kite.quote(["NSE:NIFTY 50"])
                    if "NSE:NIFTY 50" in qres:
                        spot = float(qres["NSE:NIFTY 50"].get("last_price", 0.0))
                except Exception as e:
                    print(f"[EXECUTOR] Error fetching NSE:NIFTY 50 via quote(): {e}")

        # Fallback to Yahoo Finance if API returned 0
        if spot <= 0:
            try:
                import yfinance as yf
                ticker = yf.Ticker(getattr(cfg, "YAHOO_TICKER", "^NSEI"))
        ...
        if spot > 0:
            self._cached_spot = spot
            self._cached_spot_ts = now
            try:
                import db
                db.set("last_master_spot_price", f"{spot:.2f}")
            except Exception:
                pass
        else:
            try:
                import db
                db_spot = db.get("last_master_spot_price", "0.0")
                spot = float(db_spot)
            except Exception:
                pass
    ...
    def get_mcx_futures(self, base_symbol: str) -> list:
        ...
        # Filter for MCX Futures matching base_symbol
        filtered = df[
        ...
        # Sort by expiry
        try:
            results.sort(key=lambda x: datetime.strptime(x["expiry"], "%Y-%m-%d"))
        except Exception:
